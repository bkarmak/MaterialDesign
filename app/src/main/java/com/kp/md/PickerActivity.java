...
public class PickerActivity extends AppCompatActivity {
    ...
    PickerFragment.Callback mFragmentCallback = new PickerFragment.Callback() {
        @Override
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ivLaunchPicker.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // DialogFragment to host MPicker
                PickerFragment pickerFrag = new PickerFragment();
                pickerFrag.setCallback(mFragmentCallback);

                // Options
                Pair<Boolean, MOptions> optionsPair = getOptions();

                if (!optionsPair.first) { // If options are not valid
                    Toast.makeText(PickerActivity.this, "No pickers activated",
                ...
                bundle.putParcelable("MATERIAL_OPTIONS", optionsPair.second);
                pickerFrag.setArguments(bundle);

                pickerFrag.setStyle(DialogFragment.STYLE_NO_TITLE, 0);
                pickerFrag.show(getSupportFragmentManager(), "MATERIAL_PICKER");
            }
        });
    ...
    void dealWithSavedInstanceState(Bundle savedInstanceState) {
        if (savedInstanceState == null) { // Default
            ...
            // Set callback
            PickerFragment restoredFragment = (PickerFragment)
                    getSupportFragmentManager().findFragmentByTag("MATERIAL_PICKER");
    ...
    // Validates & returns MPicker options
    Pair<Boolean, MOptions> getOptions() {
        MOptions options = new MOptions();
        int displayOptions = 0;

        if (cbDatePicker.isChecked()) {
            displayOptions |= MOptions.ACTIVATE_DATE_PICKER;
        }

        if (cbTimePicker.isChecked()) {
            displayOptions |= MOptions.ACTIVATE_TIME_PICKER;
        }

        if (cbRecurrencePicker.isChecked()) {
            displayOptions |= MOptions.ACTIVATE_RECURRENCE_PICKER;
        }

        if (rbDatePicker.getVisibility() == View.VISIBLE && rbDatePicker.isChecked()) {
            options.setPickerToShow(MOptions.Picker.DATE_PICKER);
        } else if (rbTimePicker.getVisibility() == View.VISIBLE && rbTimePicker.isChecked()) {
            options.setPickerToShow(MOptions.Picker.TIME_PICKER);
        } else if (rbRecurrencePicker.getVisibility() == View.VISIBLE && rbRecurrencePicker.isChecked()) {
            options.setPickerToShow(MOptions.Picker.REPEAT_OPTION_PICKER);
        }
